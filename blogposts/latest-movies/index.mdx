---
title: 'Latest movies with NextJS'
date: 'Jan 21, 2022'
post_id: '01212201'
hero_image: './latest-movies.jpeg'
hero_image_alt: 'Mix of movie posters'
hero_image_credit_text: 'Samuel Regan-Asante'
hero_image_credit_link: 'https://unsplash.com/photos/wMkaMXTJjlQ?utm_source=unsplash&utm_medium=referral&utm_content=creditShareLink'
---

This **TypeScript-based Next.js app**, styled with a clean responsive design based on **ChakraUI**. It is a small app I built to quickly check the latest movie releases and their ratings whenever I’m running out of ideas for what to watch.  
The app fetches data from **The Movie Database (TMDB)** via a dedicated backend API endpoint, which is part of this application. It is **client-side rendered** to ensure users always have access to the most up-to-date information.
For improved performance and scalability, **Incremental Static Regeneration (ISR)** could be implemented if hosted on Vercel, or periodic redeployments with server-side fetching could be used to refresh the data.

**Key Features**:

-   **Real-time Movie Data**: Fetches up-to-date movie information from TMDB.
-   **Responsive Design**: Built with ChakraUI for a consistent experience across devices.
-   **TypeScript**: For type safety and maintainable code.

App:
https://latest-movie-releases.netlify.app/
GitHub repo:
https://github.com/tonytettinger/Latest-movies-list-project-with-NextJS-Typescript-and-ChakraUI
